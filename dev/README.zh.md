...
**读者：** **macOS 开发者**作快速验证的**可选**路径。**生产环境与主文档以 Linux 为准** —— 请先看仓库 [README](../README.zh.md)。
...
脚本与清单在仓库目录内；**`mac.sh` 不能脱离仓库单独使用**。请先 **[clone openbkn-ai/deploy](https://github.com/openbkn-ai/deploy)**，并切换到实际部署所用分支，然后在仓库根目录执行：

```bash
git clone https://github.com/openbkn-ai/deploy.git openbkn-deploy
cd openbkn-deploy   # 在此目录执行 bash ./dev/mac.sh ...（与 deploy.sh 同层）
```

从产品包解压时，解压目录须直接包含 **`deploy.sh`**，布局与上文一致即可。
...
- **自动 IP：**`mac-config.yaml` 常用 **`accessAddress.scheme: http`**，且可**省略 `host`**（见示例）。**`openbkn install`** 会探测本机局域网 IP 并写入 values。若仅本机访问，可设 **`accessAddress.host: localhost`**。

### 操作流程

在仓库根目录执行；用 **bash** 调用（如 `bash ./dev/mac.sh ...`）。**`openbkn`** 封装默认装**全量（含必选的 bkn-safe）。
...
| 4 | `bash ./dev/mac.sh data-services install` | **可选** — 仅单独装/刷新数据层；**`openbkn install` 会先跑同一套捆绑安装**（`OPENBKN_SKIP_DATA_SERVICES_BUNDLE=true` 可跳过） |
| 5 | `bash ./dev/mac.sh openbkn download` | **可选**（本地 chart 缓存） |
| 6 | `bash ./dev/mac.sh openbkn install` | **必须** — 部署 Core（全量含 bkn-safe）；默认**先装捆绑 data-services** |
...
**最短路径：**`cluster up` → `openbkn install`（**先装数据层**）。若 **`OPENBKN_SKIP_DATA_SERVICES_BUNDLE=true`**，须自备 DB/Kafka 等可达实例，或先执行 **`data-services install`**。
...
- **`mac.sh isf install` 会自动把整套切到 HTTPS**：ISF（hydra/oauth2）的 issuer 必须是 https，所以 install 流程会自动：(1) 把 `mac-config.yaml` 的 `accessAddress` 改成 `https/443`，(2) 用 openssl 生 self-signed 证书并落到 Secret `bkn-ingress-tls`，(3) 对已装的 OpenBKN release 做 `helm upgrade` 让它们读到新 https `accessAddress`，(4) 装 ISF 并给 ingress patch TLS。全新场景大约 10 min。浏览器会提示自签证书风险，确认一次即可。**不装 ISF 的话不用动**——默认全量安装（含 bkn-safe）也跑在 HTTP 上，只有 ISF 才强制切 https。
...
- **`bkn-core-data-migrator` / Job `BackoffLimitExceeded`：**确认数据层就绪（一般由 **`openbkn install` 自动安装**；否则 **`data-services install`**）。确认 **`depServices.rds`** 指向集群内 MariaDB；必要时 `helm uninstall bkn-core-data-migrator -n <namespace>` 后再装 Core。
